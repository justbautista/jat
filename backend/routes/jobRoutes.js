...
//     await page.goto('https://www.ziprecruiter.com/jobs-search?search=software-developer&location=NY', { waitUntil: 'networkidle2' });
...
jobRoutes.get('/search_synthetic_job', async function (req, res) {
	...
	for (let job of jobPostings) {
		...
		if(title && location && company){
			if((cur_title.toLowerCase().includes(title.toLowerCase())) && (cur_company.toLowerCase().includes(company.toLowerCase())) && (cur_location.toLowerCase().includes(location.toLowerCase()))){
				j.push(job)
			}
		}
		if(title && location && !company){
			if((cur_title.toLowerCase().includes(title.toLowerCase())) && (cur_location.toLowerCase().includes(location.toLowerCase()))){
				j.push(job)
			}
		}
		if(title && !location && company){
			if((cur_title.toLowerCase().includes(title.toLowerCase())) && (cur_company.toLowerCase().includes(company.toLowerCase()))){
				j.push(job)
			}
		...
		if(!title && location && company){
			if((cur_company.toLowerCase().includes(company.toLowerCase())) && (cur_location.toLowerCase().includes(location.toLowerCase()))){
				j.push(job)
			}
		}
		if(!title && location && !company){
